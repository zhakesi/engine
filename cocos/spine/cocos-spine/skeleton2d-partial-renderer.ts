...
@ccclass('sp.Skeleton2DPartialRenderer')
@help('i18n:sp.Skeleton2DPartialRenderer')
@executionOrder(100)
@executeInEditMode
    ...
    private _assembleModel () {
        if (JSB) {
            ...
            for (let idx = 0;  idx < count; idx++) {
                ...
                const vb = new Float32Array(mesh.vCount * 9);
                for (let i = 0; i < mesh.vCount; i++) {
                    vb[i * 9] = mesh.vertices[i * 6];
                    vb[i * 9 + 1] = mesh.vertices[i * 6 + 1];
                    vb[i * 9 + 2] = mesh.vertices[i * 6 + 2];
                    vb[i * 9 + 3] = mesh.vertices[i * 6 + 3];
                    vb[i * 9 + 4] = mesh.vertices[i * 6 + 4];
                    vb[i * 9 + 5] = 1.0;
                    vb[i * 9 + 6] = 1.0;
                    vb[i * 9 + 7] = 1.0;
                    vb[i * 9 + 8] = 1.0;
